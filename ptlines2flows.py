...
def createTrips(options):
    ...
    stopsLanes = {}
    stopNames = {}
    f = codecs.open(options.ptstops, 'r', encoding = 'utf-8')
    f2 = codecs.open(options.ptlines, 'r', encoding = 'utf-8')
    for stop in sumolib.output.parse(f, 'busStop'):
    ...
    with codecs.open(options.trips, 'w', encoding="UTF8") as fouttrips:
        ...
        numStops = 0
        numSkipped = 0
        for trp_nr, line in enumerate(sumolib.output.parse(f2, 'ptLine', heterogeneous=True)):
            stop_ids = []
